...
class Settings:
    ...
    # -------------------------------------------------------------------------
    # ADDITION: Loki output settings
    # All Loki fields are optional. If loki_endpoint is empty string,
    # Loki output is disabled entirely and the exporter behaves identically
    ...
    def from_env() -> "Settings":
        ...
        gh_cost_center_ids = tuple(
            x.strip() for x in os.getenv("GH_COST_CENTER_IDS", "").split(",") if x.strip()
        )

        loki_endpoint = os.getenv("LOKI_ENDPOINT", "").rstrip("/")
        # LOKI_QUERY_URL defaults to the base of LOKI_ENDPOINT if not set.
        # e.g. if LOKI_ENDPOINT=http://loki:3100/loki/api/v1/push
        # then LOKI_QUERY_URL defaults to http://loki:3100
        loki_query_url = os.getenv("LOKI_QUERY_URL", "").rstrip("/")
        if not loki_query_url and loki_endpoint:
            # Strip /loki/api/v1/push suffix to get the base URL
            loki_query_url = loki_endpoint.replace("/loki/api/v1/push", "").rstrip("/")
...
def build_point(
    metric: str, labels: Dict[str, Any], value: Any, timestamp_ms: int
...
def day_to_midday_ns(day_str: str) -> int:
    """Nanosecond version of day_to_midday_ms. Required by Loki push API."""
    return day_to_midday_ms(day_str) * 1_000_000
...
def day_bounds_ns(day_str: str) -> Tuple[int, int]:
    """Returns (start_ns, end_ns) for a day in nanoseconds. Used by Loki dedup check."""
    d = datetime.strptime(day_str, "%Y-%m-%d").date()
...
class GitHubClient:
    ...
    def _safe_billing_get(
        self, url: str, params: Dict[str, Any], empty_payload: Dict[str, Any]
    ) -> Dict[str, Any]:
        resp = self.api.get(url, params=params, timeout=DEFAULT_HTTP_TIMEOUT)
    ...
    def fetch_org_billing_usage(
        self, org: str, kind: str, year: int, month: int, day: Optional[int] = None
    ) -> Dict[str, Any]:
        if kind not in {"premium_request", "ai_credit"}:
            raise ValueError(f"Unsupported billing kind: {kind}")
...
class VictoriaMetricsClient:
    ...
    def export_existing_points(
        ...
        day: str,
        extra_match_labels: Optional[Dict[str, str]] = None,
    ) -> Dict[str, float]:
...
# -----------------------------------------------------------------------------
# ADDITION: Loki client
# Pushes one structured JSON log line per user per day.
# Activated only when settings.loki_endpoint is non-empty.
# Loki failures are logged and counted but never raise — the exporter cycle
# continues even if Loki is down.
# -----------------------------------------------------------------------------
...
class LokiClient:
    def __init__(self, settings: Settings):
        self.settings = settings
        self.http = requests.Session()
        self.enabled = False  # set to True after successful health check

    def check_health(self) -> bool:
        """
        Call once at startup. Sets self.enabled.
        Returns True if Loki is reachable.
        Does not raise.
        """
        if not self.settings.loki_endpoint:
            logging.info("loki: LOKI_ENDPOINT not set — Loki output disabled")
            self.enabled = False
            return False

        ready_url = f"{self.settings.loki_query_url}/ready"
        try:
            resp = self.http.get(ready_url, timeout=5)
            if resp.status_code == 200:
                logging.info("loki: health check OK (%s) — Loki output enabled", ready_url)
                self.enabled = True
                return True
            logging.warning(
                "loki: /ready returned HTTP %d — Loki output disabled", resp.status_code
            )
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        if self.settings.loki_tenant_id:
            h["X-Scope-OrgID"] = self.settings.loki_tenant_id
        return h

    def day_already_present(self, day: str) -> bool:
        """
        One GET to Loki query_range API per day to guard against double-push.
        Returns True if any log lines for this enterprise+day already exist.
        On any error, returns False (attempt the push — idempotent).
        """
        if not self.settings.loki_query_url:
            return False
        start_ns, end_ns = day_bounds_ns(day)
        query = f'{{enterprise="{self.settings.gh_enterprise}",log_source="copilot_exporter"}}'
        try:
            resp = self.http.get(
                f"{self.settings.loki_query_url}/loki/api/v1/query_range",
                params={
                    "query": query,
                    "start": str(start_ns),
                    "end": str(end_ns),
                ...
                timeout=self.settings.loki_timeout_sec,
            )
            resp.raise_for_status()
            ...
            logging.warning(
                "loki: dedup check failed for day=%s, will attempt push: %s", day, exc
            )
            return False

    def _build_stream(self, user_row: Dict[str, Any], day: str) -> Dict[str, Any]:
        """
        Build one Loki stream entry for a single users-1-day NDJSON record.

        Stream labels (low-cardinality — 5 labels):
          enterprise, user_login, ai_adoption_phase, log_source, deployment_env

        Log body: JSON string parseable with | json in LogQL.
        Timestamp: noon UTC of the report day in nanoseconds (string),
          matching day_to_midday_ms convention used throughout the exporter.
        """
        login = str(user_row.get("user_login") or "unknown")

        # Normalise ai_adoption_phase — may be a string or a dict with name/version
        ...
        # CLI token fields — nested under totals_by_cli.token_usage
        ...
        body: Dict[str, Any] = {
            ...
            "code_generation_activity_count": user_row.get("code_generation_activity_count"),
            "code_acceptance_activity_count": user_row.get("code_acceptance_activity_count"),
            "loc_suggested_to_add_sum": user_row.get("loc_suggested_to_add_sum"),
            "loc_suggested_to_delete_sum": user_row.get("loc_suggested_to_delete_sum"),
            ...
            # Breakdown summaries (arrays — filterable with | json in LogQL)
            "totals_by_model_feature": self._summarise_model_feature(user_row),
            "totals_by_feature": self._summarise_feature(user_row),
            "totals_by_ide": self._summarise_ide(user_row),
            "totals_by_language_feature": self._summarise_language_feature(user_row),
        }

        # Loki timestamp must be a STRING in nanoseconds per the push API spec.
        ts_ns_str = str(day_to_midday_ns(day))
        ...
        if self.settings.deployment_env:
            stream_labels["deployment_env"] = self.settings.deployment_env

        return {
            "stream": stream_labels,
            "values": [[ts_ns_str, json.dumps(body, default=str)]],
        }
    ...
    def _summarise_model_feature(row: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for item in row.get("totals_by_model_feature") or []:
            ...
            agg[key]["prompt_count"] += int(item.get("user_initiated_interaction_count") or 0)
            agg[key]["loc_added_sum"] += int(item.get("loc_added_sum") or 0)
        return sorted(agg.values(), key=lambda x: x["prompt_count"], reverse=True)
    ...
    def _summarise_feature(row: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for item in row.get("totals_by_feature") or []:
            ...
            agg[feat]["prompt_count"] += int(item.get("user_initiated_interaction_count") or 0)
    ...
    def _summarise_ide(row: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for item in row.get("totals_by_ide") or []:
            ...
            agg[ide]["prompt_count"] += int(item.get("user_initiated_interaction_count") or 0)
            agg[ide]["loc_added_sum"] += int(item.get("loc_added_sum") or 0)
        return sorted(agg.values(), key=lambda x: x["prompt_count"], reverse=True)

    @staticmethod
    def _summarise_language_feature(row: Dict[str, Any]) -> List[Dict[str, Any]]:
        agg: Dict[str, Dict[str, Any]] = {}
    ...
    def push_user_day(self, user_rows: List[Dict[str, Any]], day: str) -> int:
        """
        Push all user rows for a given day to Loki.
        - Checks dedup first (one GET per day, not per user).
        - Batches into loki_batch_size streams per HTTP POST.
        - Returns total lines pushed.
        - Never raises — errors are logged and counted in LOKI_PUSH_ERRORS.
        """
        if not self.enabled:
            return 0

        if self.day_already_present(day):
            logging.info(
                "loki: day=%s already present for enterprise=%s — skipping push",
                day, self.settings.gh_enterprise,
            )
            return 0
        ...
        for row in user_rows:
            try:
                streams.append(self._build_stream(row, day))
            except Exception as exc:
                logging.warning(
                    "loki: failed to build stream for user=%s day=%s: %s",
                    row.get("user_login", "?"), day, exc,
                )
        ...
        for batch_start in range(0, len(streams), batch_size):
            batch = streams[batch_start: batch_start + batch_size]
            payload = json.dumps({"streams": batch}, default=str)
            try:
                resp = self.http.post(
                    self.settings.loki_endpoint,
                    data=payload,
                    headers=headers,
                    timeout=self.settings.loki_timeout_sec,
                )
                if resp.status_code in (200, 204):
                    ...
                    LOKI_PUSHED_LINES.labels(enterprise=self.settings.gh_enterprise).inc(len(batch))
                    logging.debug(
                        "loki: pushed %d lines day=%s batch_start=%d",
                        len(batch), day, batch_start,
                    )
                else:
                    LOKI_PUSH_ERRORS.labels(enterprise=self.settings.gh_enterprise).inc()
                    logging.error(
                        "loki: HTTP %d for day=%s batch_start=%d: %s",
                        resp.status_code, day, batch_start, resp.text[:300],
                    )
        ...
        logging.info(
            ...
            day, self.settings.gh_enterprise, total_pushed, len(streams),
        )
        return total_pushed
...
def build_enterprise_usage_points(row: Dict[str, Any], settings: Settings) -> List[MetricPoint]:
    ...
    for cohort in row.get("totals_by_ai_adoption_phase") or []:
        # GitHub API may return ai_adoption_phase as a dict
        # e.g. {'phase_number': 0, 'phase': 'No Cohort', 'version': 'v1'}
        raw_phase = cohort.get("ai_adoption_phase", "unknown")
        if isinstance(raw_phase, dict):
            phase_str = str(raw_phase.get("phase", "unknown"))
        else:
            phase_str = str(raw_phase)
        labels = {
            **base,
            "ai_adoption_phase": phase_str,
        }
        cohort_fields = {
    ...
    cli_fields = {
        ...
        # ADDITION 3: avg_total_tokens_per_session present in API, not previously emitted
        "github_copilot_cli_avg_total_tokens_per_session": cli_token.get("avg_total_tokens_per_session"),
    }
...
def build_user_usage_points(row: Dict[str, Any], settings: Settings) -> List[MetricPoint]:
    ...
    # ADDITION 1: ai_credits_used — June 19 2026 field.
    # Overall per-user daily total across all Copilot surfaces.
    # 1 AI Credit = $0.01 USD.
    ai_credits = row.get("ai_credits_used")
    if ai_credits is not None:
        p = build_point("github_copilot_user_ai_credits_used", base, ai_credits, ts_ms)
        if p:
            points.append(p)
    ...
    if ai_adoption_phase:
        # GitHub API may return ai_adoption_phase as a dict
        # e.g. {'phase_number': 0, 'phase': 'No Cohort', 'version': 'v1'}
        # Extract the 'phase' string to keep the label clean and queryable.
        if isinstance(ai_adoption_phase, dict):
            ai_adoption_phase_str = str(ai_adoption_phase.get("phase", "unknown"))
        else:
            ai_adoption_phase_str = str(ai_adoption_phase)
        labels = {**base, "ai_adoption_phase": ai_adoption_phase_str}
    ...
    # ADDITION 2: user-level CLI token fields.
    # The production exporter read last_known_cli_version for the version label
    # but did not parse token_usage. Adding output_tokens_sum, prompt_tokens_sum,
    # avg_tokens_per_request from totals_by_cli.token_usage.
    ...
    if isinstance(last_known_cli_version, dict):
        ...
        if cli_version:
            cli_labels = {**base, "cli_version": cli_version}
            p = build_point("github_copilot_user_cli_daily_record", cli_labels, 1, ts_ms)
            if p:
                points.append(p)

            # Token fields — nested inside token_usage sub-object
            cli_token = totals_by_cli.get("token_usage") or {}
            for metric_name, value in {
                "github_copilot_user_cli_output_tokens_sum": cli_token.get("output_tokens_sum"),
                "github_copilot_user_cli_prompt_tokens_sum": cli_token.get("prompt_tokens_sum"),
                "github_copilot_user_cli_avg_tokens_per_request": cli_token.get("avg_tokens_per_request"),
            }.items():
                p = build_point(metric_name, cli_labels, value, ts_ms)
                if p:
                    points.append(p)
...
def build_enterprise_seat_points(
    seat_payload: Dict[str, Any], settings: Settings, target_date: date
) -> List[MetricPoint]:
    """
...
def build_billing_points(
    ...
    for item in usage_items:
        ...
        for field_name, value in item.items():
            if field_name in seen_fields or field_name in {
                "model", "unitType", "sku", "product", "pricePerUnit",
                "organization", "organizationName", "repositoryName",
            }:
                continue
            num = coerce_number(value)
            if num is None:
...
def build_billing_usage_summary_points(
    ...
    for item in payload.get("usageItems", []):
        ...
        for field_name in [
            "quantity", "grossQuantity", "grossAmount",
            "discountQuantity", "discountAmount", "netQuantity", "netAmount",
        ]:
            value = item.get(field_name)
            num = coerce_number(value)
            if num is None:
...
class CopilotExporter:
    def __init__(self, settings: Settings):
        ...
        self.loki = LokiClient(settings)  # ADDITION: Loki client

        self.last_daily_import_day: Optional[str] = None
        self.bootstrapped = False
        self.backfill_done = False

    def _import_family_points(
        self, family: str, day: str, points: Sequence[MetricPoint]
    ) -> int:
        missing, skipped, drift = diff_points_against_vm(
            self.vm, points, day, family, self.settings
        )
        ...
        IMPORTED_POINTS.labels(
            enterprise=self.settings.gh_enterprise, family=family
        ).inc(imported)
    ...
    def _process_daily_usage(self, day: str) -> int:
        ...
        # ADDITION: Push user rows to Loki after VM write.
        # Only runs if loki.enabled is True (set by check_health() at startup).
        # DRY_RUN does not suppress Loki — Loki has its own dedup check.
        # If you want DRY_RUN to also skip Loki, wrap the call in:
        #   if not self.settings.dry_run:
        if user_rows:
            self.loki.push_user_day(user_rows, day)
    ...
    def _process_daily_billing(self, day: str) -> int:
        ...
        for kind in ("premium_request", "ai_credit"):
            if self.settings.billing_scope == "enterprise":
                aggregate = self.github.fetch_enterprise_billing_usage(
                    kind, d.year, d.month, d.day
                )
                aggregate_points = build_billing_points(
                    "all", aggregate, kind, self.settings, source_scope="enterprise"
                )
                total += self._import_family_points(
                    f"billing_{kind}:all", day, aggregate_points
                )

                for org in self.settings.gh_billing_orgs:
                    payload = self.github.fetch_enterprise_billing_usage(
                        kind, d.year, d.month, d.day, organization=org
                    )
                    points = build_billing_points(
                        org, payload, kind, self.settings,
                        source_scope="enterprise", cost_center_id=None,
                    )
                    total += self._import_family_points(
                        f"billing_{kind}:{org}", day, points
                    )
            else:
                for org in org_targets:
                    if org == "all":
                        continue
                    payload = self.github.fetch_org_billing_usage(
                        org, kind, d.year, d.month, d.day
                    )
                    points = build_billing_points(
                        org, payload, kind, self.settings, source_scope="organization"
                    )
                    total += self._import_family_points(
                        f"billing_{kind}:{org}", day, points
                    )
        ...
        if self.settings.enable_billing_usage_summary:
            summary_payload = self.github.fetch_enterprise_billing_usage_summary(
                d.year, d.month, d.day
            )
            summary_points = build_billing_usage_summary_points(
                summary_payload, self.settings
            )
            total += self._import_family_points(
                "billing_usage_summary:all", day, summary_points
            )
            for cost_center_id in self.settings.gh_cost_center_ids:
                payload = self.github.fetch_enterprise_billing_usage_summary(
                    d.year, d.month, d.day, cost_center_id=cost_center_id
                )
                points = build_billing_usage_summary_points(
                    payload, self.settings, cost_center_id=cost_center_id
                )
                total += self._import_family_points(
                    f"billing_usage_summary:cost_center:{cost_center_id}", day, points
                )
    ...
    def bootstrap_28d_once(self) -> None:
        ...
        logging.info(
            "Starting exact-diff 28-day bootstrap for enterprise=%s",
        ...
        for day in all_days:
            ...
            for row in enterprise_by_day.get(day, []):
                enterprise_points.extend(
                    build_enterprise_usage_points(row, self.settings)
                )
            self._import_family_points("enterprise_usage", day, enterprise_points)

            day_user_rows = user_by_day.get(day, [])
            user_points: List[MetricPoint] = []
            for row in day_user_rows:
                user_points.extend(build_user_usage_points(row, self.settings))
            self._import_family_points("user_usage", day, user_points)

            # ADDITION: Push each bootstrap day to Loki
            if day_user_rows:
                self.loki.push_user_day(day_user_rows, day)

        self.bootstrapped = True
        if all_days:
            self.last_daily_import_day = max(all_days)
        logging.info(
            "Completed exact-diff 28-day bootstrap for enterprise=%s latest_day=%s",
            self.settings.gh_enterprise,
            self.last_daily_import_day,
        )
    ...
    def backfill_date_range_once(self) -> None:
        ...
        if not self.settings.backfill_start_day or not self.settings.backfill_end_day:
            raise RuntimeError(
                "Backfill requested but BACKFILL_START_DAY/BACKFILL_END_DAY is missing"
            )
    ...
    def import_latest_stable_day_if_needed(self) -> None:
        target_day = (
            ...
            - timedelta(days=self.settings.data_lag_days)
        ).isoformat()
        if self.last_daily_import_day == target_day:
            logging.info(
                "Stable day %s already processed in this pod lifecycle; skipping",
                target_day,
            )
            return
        ...
        logging.info(
            "Completed exact-diff stable day import enterprise=%s day=%s",
            self.settings.gh_enterprise,
            target_day,
        )
    ...
    def run_cycle(self) -> None:
        ...
        try:
            ...
            logging.exception(
                "Collector cycle failed for enterprise=%s", self.settings.gh_enterprise
            )
            raise
        finally:
            LAST_DURATION.labels(enterprise=self.settings.gh_enterprise).set(
                time.time() - start
            )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--start-day",
        help="Run an exact-diff backfill / import from this day (YYYY-MM-DD)",
    )
    parser.add_argument(
        "--end-day",
        help="Run an exact-diff backfill / import through this day (YYYY-MM-DD)",
    )
    parser.add_argument(
        "--once",
        action="store_true",
        help="Run a single collector cycle and exit",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # ADDITION: Loki health check at startup.
    # Sets exporter.loki.enabled. Does not block startup if Loki is down.
    exporter.loki.check_health()
