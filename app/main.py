...
def fetch_org_users_usage_28d() -> List[Any]:
    url = f"{GH_API_BASE}/orgs/{GH_ORG}/copilot/metrics/reports/users-28-day/latest"
    meta = github_get_json(url)
    return download_report_chunks(meta.get("download_links", []))


def fetch_org_users_usage_for_day(day: str) -> List[Any]:
    url = f"{GH_API_BASE}/orgs/{GH_ORG}/copilot/metrics/reports/users-1-day"
    meta = github_get_json(url, params={"day": day})
    return download_report_chunks(meta.get("download_links", []))
...
def build_org_usage_series(row: Dict[str, Any]) -> List[str]:
    ...
    for item in row.get("totals_by_feature") or []:
        labels = {"org": GH_ORG, "feature": str(item.get("feature", "unknown"))}
    ...
    for item in row.get("totals_by_ide") or []:
        labels = {"org": GH_ORG, "ide": str(item.get("ide", "unknown"))}
...
def bootstrap_28d_once():
    ...
    org_chunks = fetch_org_usage_28d()
    org_rows = extract_rows(org_chunks)

    user_chunks = fetch_org_users_usage_28d()
    ...
    for row in org_rows:
        ...
        lines.extend(build_org_usage_series(row))
...
def import_latest_stable_day_if_needed():
    ...
    org_chunks = fetch_org_usage_for_day(target_day)
    org_rows = extract_rows(org_chunks)

    user_chunks = fetch_org_users_usage_for_day(target_day)
    user_rows = extract_rows(user_chunks)
