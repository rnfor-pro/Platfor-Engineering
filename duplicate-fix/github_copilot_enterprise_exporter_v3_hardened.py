...
class MetricPoint:
    ...
    def series_key(self) -> str:
        """
        Identifies a unique (metric, labels, timestamp) position WITHOUT the value.

        Two points with the same metric+labels+timestamp but different values are the
        SAME series position. They must produce the same key so the diff engine treats
        the second as drift (one value wins per drift_policy) rather than a new insert.

        Previously this delegated to record_key() which included the value. That caused
        two writes of the same position (e.g. enterprise cohort totals + per-user fallback
        rollup) to get different keys, both pass "not in existing", and both land in VM.
        Grafana then summed them producing 2x-30x inflated counts.
        """
        payload = {
            "metric": self.metric,
    ...
    def record_key(self) -> str:
        """
        Full content hash including value. Used only for intra-batch dedup
        to avoid sending identical rows twice in one import call.
        NOT used for cross-run deduplication — use series_key() for that.
        """
        payload = {
            "metric": self.metric,
            "labels": list(self.canonical_labels()),
...
class GitHubClient:
    ...
    def fetch_enterprise_seats(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            # GitHub seat API page size is 100. Break only when the page is
            # empty, not when it has fewer than 50 rows -- the old threshold
            # silently dropped the last page for enterprises whose seat count
            # modulo 100 falls between 1 and 49.
            if not page_rows:
                break
            page += 1
...
def build_enterprise_seat_points(
    ...
    # FIX 4: Freeze reference timestamp to collected_at (captured BEFORE the
    # fetch_enterprise_seats() API call). For large enterprises the pagination
    # can take 5-30s, so using datetime.now() inside the loop means later pages
    # are evaluated against a staler timestamp than earlier pages, flipping
    # boundary seats across the 7d/28d/90d thresholds between pages.
    now_ts = (collected_at or datetime.now(timezone.utc)).timestamp()

    # FIX 3: Deduplicate seats by user_login for summary counters.
    # GitHub's enterprise seat API can return the same user_login on multiple
    # rows when the user has seats in more than one org under the enterprise.
    # Without dedup those users are counted N times in active/never/pending
    # totals, making the bucket sums exceed total_seats.
    # We still write the full per-seat metric row for each org occurrence
    # (preserving label cardinality) but only count the user once for summaries.
    seen_logins_for_summary: set = set()
    ...
    for seat in seats:
        ...
        # Per-org plan/team counters increment for every seat row (each org counts)
        ...
        # Summary counters: count each distinct user_login only once
        if user_login not in seen_logins_for_summary:
            seen_logins_for_summary.add(user_login)
            if last_activity_at is None:
                never_active += 1
            else:
                delta_days = (now_ts - last_activity_at) / 86400.0
                if delta_days <= 7:
                    active_last_7d += 1
                if delta_days <= 28:
                    active_last_28d += 1
...
def build_billing_points(
    ...
    # FIX 8: Deduplicate usageItems by content before processing.
    # GitHub's billing API has been observed returning identical rows in the
    # same response for some SKU/model combinations. Without dedup, the total_
    # aggregates double-count those rows and per-item series_key collisions
    # produce intra-batch duplicates.
    seen_item_keys: set = set()
    unique_usage_items = []
    for raw_item in (payload.get("usageItems") or []):
        item_key = json.dumps(raw_item, sort_keys=True, separators=(",", ":"))
...
def build_billing_usage_summary_points(
    ...
    totals: Dict[str, float] = {}
    seen_summary_keys: set = set()
    unique_summary_items = []
    for raw_item in (payload.get("usageItems") or []):
        item_key = json.dumps(raw_item, sort_keys=True, separators=(",", ":"))
        if item_key not in seen_summary_keys:
            seen_summary_keys.add(item_key)
            unique_summary_items.append(raw_item)
        else:
            logging.warning(
                "Duplicate usageItem in billing_usage_summary payload day=%s -- dropping duplicate",
                target_day,
            )
    for item in unique_summary_items:
        labels = dict(base)
...
def diff_points_against_vm(
    ...
    # FIX 5: Deduplicate within the incoming batch before comparing to VM.
    # If the API returns duplicate rows (observed for billing usageItems and
    # possible for enterprise-1-day rows spanning pagination boundaries),
    # two points can share a series_key in one batch. Without intra-batch
    # dedup both pass "not in existing" and both get written to VM.
    # Last writer per series_key wins; the drop is logged as a warning.
    ...
    if intra_batch_dupes:
        logging.warning(
            "Dropped %s intra-batch duplicate points family=%s day=%s before VM comparison -- "
            "check for API-level duplicate rows or double-write in builder functions",
            intra_batch_dupes, family, day,
        )
    unique_points = list(deduped.values())

    metric_names = sorted({p.metric for p in unique_points})
...
class CopilotExporter:
    ...
    def _process_daily_usage(self, day: str) -> int:
        ...
        # FIX 2: Gate the per-user cohort fallback on whether the enterprise report
        # already provided totals_by_ai_adoption_phase.
        #
        # Both sources write github_copilot_ai_adoption_phase_user_count with the
        # same labels and timestamp but DIFFERENT values (GitHub's 28d-window server
        # count vs the exporter's daily distinct-user count). The old series_key bug
        ...
        if not enterprise_had_cohort_totals:
            ...
            logging.info(
                "Built cohort_rollup points day=%s count=%s (fallback: no enterprise cohort totals for this day)",
                day, len(cohort_rollup_points),
            )
        else:
            cohort_rollup_points = []
            logging.info(
                "Skipping cohort_rollup fallback day=%s: enterprise rows contain authoritative cohort totals",
                day,
            )
    ...
    def _process_daily_billing(self, day: str) -> int:
        ...
        # FIX 7: Warn when billing_scope changes so operators know that old
        # data written under a different scope label still exists in VM and
        # will be silently included in dashboard sums unless deleted.
        # The series for billing_scope="enterprise" and billing_scope="organization"
    ...
    def bootstrap_28d_once(self) -> None:
        ...
        for day in all_days:
            e_rows = enterprise_by_day.get(day, [])
            u_rows = user_by_day.get(day, [])
            ...
            if not bootstrap_had_cohort_totals:
                ...
                self._import_family_points("cohort_rollups", day, cohort_pts)

        self.bootstrapped = True
        # FIX 11: Do NOT set last_daily_import_day to the bootstrap latest day.
        # If that day equals today minus DATA_LAG_DAYS, the stable-day import
        # skips for the entire pod lifecycle missing fresh data GitHub publishes.
        # Leave it as None so the stable-day import always runs after bootstrap.
        logging.info(
            "Completed exact-diff 28-day bootstrap for enterprise=%s days=%s "
            "(last_daily_import_day intentionally NOT set to avoid blocking stable day import)",
            self.settings.gh_enterprise, len(all_days),
        )
