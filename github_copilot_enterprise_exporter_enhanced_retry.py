...
class GitHubClient:
    ...
    def download_chunks(
        ...
        """
        Download signed GitHub report chunks.

        Some signed report URLs can return 403 if they expire or become invalid
        between metadata retrieval and file download. When that happens, try once
        to fetch a fresh metadata response and re-download with the new signed
        links before failing the cycle.
        """
        ...
        for attempt in range(2):
            ...
            try:
                for idx, link in enumerate(links):
                    ...
                    if resp.status_code == 403 and refresh_links_fn is not None and attempt == 0:
                        raise requests.HTTPError("403 signed url expired", response=resp)
                    resp.raise_for_status()
                ...
                if (
                    attempt == 0
                    and refresh_links_fn is not None
                    and response is not None
                    and response.status_code == 403
                ):
                    logging.warning(
                        "Signed report download returned 403 for report=%s. Refreshing metadata and retrying once.",
...
def build_user_team_points(row: Dict[str, Any], settings: Settings) -> List[MetricPoint]:
    ...
    user_login = str(row.get("user_login", "unknown"))
    user_id = str(row.get("user_id", "unknown"))
    team_slug = str(row.get("slug") or row.get("team_slug") or row.get("team") or "unknown")
...
def build_ai_adoption_phase_rollup_points(user_rows: Sequence[Dict[str, Any]], settings: Settings) -> List[MetricPoint]:
    ...
    for row in user_rows:
        day = row.get("day")
        phase = str(row.get("ai_adoption_phase") or "").strip()
...
def build_team_rollup_points(
    ...
    for team_row in team_rows:
        ...
        team_id = str(team_row.get("team_id") or "unknown")
        team_slug = str(team_row.get("slug") or team_row.get("team_slug") or team_row.get("team") or "unknown")
